...
TRADES_MAIN = os.path.join(LOG_DIR, "v35_trades_main.csv")
HUNT_LOG = os.path.join(LOG_DIR, "v35_hunt_log.csv")

def calculate_score(pnl, trade_count, pf, wr, mdd, mae, regret, capture):
    # V35 공식 (V34와 동일한 평가지표 유지)
...
def check_results():
    print("\n" + "="*160)
    print(f"  [V35 Long/Short Sniper 실시간 랭킹 모니터링]  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*160)
